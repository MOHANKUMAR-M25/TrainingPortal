...
export default function CalendarBooking({ consultation }) {
  ...
  // Pre-fill the booking form from the logged-in user's profile
  useEffect(() => {
    if (user) {
      setForm((prev) => ({
        ...prev,
  ...
  const handleBook = async (e) => {
    ...
    try {
      ...
      // Keep the profile details for signed-in users; only reset the rest
      setForm({
        name: user?.name || "",
        email: user?.email || "",
        phone: user?.phone || "",
        ...
        <form onSubmit={handleBook} className="mt-8 grid gap-4 border-t border-slate-100 pt-6 sm:grid-cols-2">
          ...
          {user && (
            <p className="rounded-xl bg-green-50 p-3 text-xs text-green-700 sm:col-span-2">
              ✅ Booking as <span className="font-bold">{user.name || user.email}</span> — your details are filled in
