...
export default function Courses({ courses, onContentChanged }) {
  ...
  const handleEnrollPay = async (e) => {
    e.preventDefault();
    const amount = priceToNumber(enrollCourse.price);
    if (amount < 1) {
    ...
    try {
      const result = await payWithRazorpay({
        ...
        onFailure: (err) =>
          setEnrollStatus({ type: "error", message: "Payment failed: " + err.message + " — no money deducted." }),
        onDismiss: () => setEnrollStatus({ type: "error", message: "Payment cancelled. Enrollment not completed." })
      });
      if (result.success) {
        setEnrollStatus({
          type: "success",
          message: `🎉 Payment successful! You're enrolled in ${enrollCourse.title}. A confirmation has been emailed to you.`
        });
  ...
  return (
    <section id="courses" className="bg-slate-50 py-20">
      ...
      {/* Enroll & Pay modal */}
      {enrollCourse && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/60 p-4" onClick={() => !paying && setEnrollCourse(null)}>
          <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="font-display text-xl font-bold">Enroll in {enrollCourse.level}</h3>
              <button onClick={() => !paying && setEnrollCourse(null)} className="text-slate-400 hover:text-slate-700">✕</button>
            </div>
            <p className="mt-1 text-sm text-slate-500">{enrollCourse.title}</p>
            <p className="mt-2 font-display text-3xl font-extrabold text-german-red">{enrollCourse.price}</p>

            <form onSubmit={handleEnrollPay} className="mt-5 grid gap-3">
              <input className="input-field" placeholder="Your name *" value={enrollForm.name} onChange={(e) => setEnrollForm({ ...enrollForm, name: e.target.value })} required />
              <input className="input-field" type="email" placeholder="Your email *" value={enrollForm.email} onChange={(e) => setEnrollForm({ ...enrollForm, email: e.target.value })} required />
              <input className="input-field" placeholder="Phone / WhatsApp *" value={enrollForm.phone} onChange={(e) => setEnrollForm({ ...enrollForm, phone: e.target.value })} required />
              <button type="submit" disabled={paying} className="btn btn-primary disabled:opacity-60">
                {paying ? "Opening payment…" : `Pay ${enrollCourse.price} & Enroll`}
              </button>
